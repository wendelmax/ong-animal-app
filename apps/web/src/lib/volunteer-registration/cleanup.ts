...
export async function cleanupOrphanedVolunteerFiles(payload: any, r2: R2StoragePort, now = new Date(), retentionHours = 24) {
  ...
  const result = await payload.find({ collection: 'volunteer-files', where: { uploadStatus: { equals: 'UPLOADED' }, volunteer: { exists: false }, createdAt: { less_than: cutoff } }, limit: 100, overrideAccess: true })
  let deleted = 0
  let failed = 0
