...
const ipFrom = (req: any) => req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || req.headers.get('x-real-ip') || 'unknown'
...
const rateLimit = async (req: any, token: string) => {
  const result = await checkPublicRateLimit({ key: `invite:${token}:${ipFrom(req)}`, limit: 30, windowSeconds: 3600 })
  if (!result.success) throw new VolunteerRegistrationError('RATE_LIMITED', 429)
}
